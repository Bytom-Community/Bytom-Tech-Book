...
## 3.2 bytomd Commandline flags (bytomd命令flag参数详解)
   ...
   Commandline programs accept input arguments. Programming lanaguages offer functions and libraries to parse these input arguments. In GoLang, 'flag' package handles parsing the input arguments. 
   Sub-commands and other parameters supported by bytomd are as follows:
​	
...
​	Daemon is a special kind of process, which keeps on running in the background after it starts and it does not stop until it receives a `kill term` signal.
   ...
   The directory contents of bytomd looks as follows: 
...
​	`node.NewNode(config)` creates a new `Node` object, which contains all the functionality of bytomd.

​	`cmn.BaseService` is a service management framework based on tendermint. `Node` object calls these methods `OnStart/OnStop/IsRunning` on BaseService in order to manage node lifecycle. Tendermint ensures these operations are not run repeatedly.
...
***Note***: In Bitcoin, nodes use DNS to find the seed node and query it to get the IP addresses of other nodes. In Bytom, seed node IP addresses are hard coded into the code. We will explain this in more detail in ***Chapter10 P2P Network***.
...
​	In `init()` method, there are different types of flags which are bound to `config`. Here is an example: 
...
### 3.4.3 File Lock (创建文件锁)

​	In this section, We will look into what happens after initializaiton is complete and the the program starts running.

​	In Bytom, a data directory(specified by `--root`) can only be read and written by one bytom daemon at a time. This is because the LevelDB Key-Value store runs in a single process. If multiple processes write to the same file, the consistency of data cannot be ensured. To ensure only one process reads or writes the data file at a time, bytom uses a file lock.
...
​	When bytomd starts, `lockDataDirectory()` function uses `flock` to create a `LOCK` file in the `RootDir` Directory. If one bytomd process locks the `inode` of a file during the start, then any other process which tries to start at the same time, will get an error message as defined in `errors.New` and this new process will exit failing to aquire the `LOCK`. `flock` is used for detecting if a bytomd process is already running.
...
* Bech32HRPSegw：Segregated Witness, an upgrade of protocol. See more details in Chapter-05 about Kernel Layer.
...
​	All data (blocks, transactions, etc.) on the blockchain needs to be stored in local K/V database once a pubilc blockchain is created. Bytom uses LevelDB as it's database.
...
（2）`store.GetStoreStatus()` can gets the storage status of local blockchain. If it is nil, the blockchain hasn't been initialized and so it runs `initChainStatus` to add the genesis block to initialize local blockchain.
...
* netsync.NewSyncManager：Instantiates `syncManager` object, which synchronizes information of blocks and transactions among nodes.
* newPoolTxListenner：is run as a goroutine amd listens to transactions in TxPool and send transactions to `syncManager` object or local wallet.
...
In bytom, `ppof` is disabled by default, users can enable it by `--prof_laddr`. 

Here is the code that initializes pprof over http:
